...
| Then, no owner input needed | The client's path to the backend for ticket search and publishing (a pipe message and the service's HTTP call, like drafting), then **ST-093/094** note and time-entry push; ST-009 (vault), ST-078 (publish panel) and ST-091/092 (ConnectWise, against recorded shapes) are done | Agent |
